...
WL.registerComponent('mesh-particles', {
    ...
    update: function (dt) {
        ...
        if (spawnParticles && !this.firstTime) {
            this.firstTime = true;

            PP.EasyTuneVariables.addVariable(new PP.EasyTuneNumber("Particle Size", 0.005, 0.01, 4));
            PP.EasyTuneVariables.addVariable(new PP.EasyTuneNumber("Particle Speed", 0.5, 0.5, 2));
        }

        if (this.firstTime) {
            this.particleScale = PP.EasyTuneVariables.get("Particle Size").myValue;
            this.initialSpeed = PP.EasyTuneVariables.get("Particle Speed").myValue;
        }
        ...
        this.object.setTranslationLocal([Math.sin(this.totalTime * 8) * 0.075, 0.025, Math.cos(this.totalTime * 8) * 0.075]);
        ...
        for (let i = 0; i < Math.min(this.count, this.objects.length); ++i) {
            ...
            if ((origin[1] + vel[1] * dt) <= this.particleScale - 1 && vel[1] <= 0) {
                /* Pseudo friction */
